<template>
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Laporan Kinerja instruktur</h1>
    </div>
                ...
                <div class="card border-0 rounded shadow">
                    ...
                    <div class="card-body" ref='content'> 
                        ...
                        <table class="table table-striped table-bordered mt-4" id="table">                           
                            ...
                            <tbody>
                                <tr v-for="(lapKinerja, id) in lapKinerjas" :key="id">
                                    <td>{{ lapKinerja.nama}}</td>
                                    <td>{{ lapKinerja.jumlah_hadir}}</td>
                                    <td>{{ lapKinerja.jumlah_libur}}</td>
                                    <td>{{ lapKinerja.keterlambatan}}</td>
                                </tr>
                            </tbody> 
                        </table>                        
                    </div> 
...
export default { 
    setup() { 
        ...
        function laporanKelasGet(bulan_temp){
            ...
            axios.get('https://api-gofit.200710947.com/api/laporan/kinerja/' + bulan_temp) 
            .then(response => { 
                //assign state posts with response data 
                lapKinerjas.value = response.data.data 
                bulan.value = response.data.bulan
                tahun.value = response.data.tahun 
                tanggal_cetak.value = response.data.tanggal_cetak 
                console.log(lapKinerjas)
            }).catch(error => {
        ...
        function cetakLaporan($tahun, $tanggal_cetak, $bulan) {
        ...
        return { lapKinerjas, bulan, tahun, tanggal_cetak, laporanKelasGet, cetakLaporan} },
